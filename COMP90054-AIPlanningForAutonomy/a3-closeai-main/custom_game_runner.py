...
def loadAgent(matches, superQuiet=True, reinitialize_first=True):
    teams = matches['teams']
    num_of_agents = len(teams)
    agents = [None]*num_of_agents
    valid_game = True
    for i in range(num_of_agents):
        if i == 1 and not reinitialize_first:  # Check if it's the second agent and we are retaining it
            continue  # Skip reinitialization of the second agent
...
def run(options,msg):
    ...
    seed_list = [random.randint(0,1e10) for _ in range(10000)]
    seed_idx = 0
    ...
    loaded_agents, valid_game = loadAgent(matches, superQuiet=options.superQuiet, reinitialize_first=True)
    dqn_agent_0 = loaded_agents[0]
    dqn_agent_1 = loaded_agents[1]
    for game_num in range(options.multipleGames):
        game = {}
        #if game_num != 0:  # Only on the first game, we load both agents
        #    loaded_agents, valid_game = loadAgent(matches, superQuiet=options.superQuiet, reinitialize_first=False)
        loaded_agents = [dqn_agent_0, dqn_agent_1]
